...
def show_national_cases(national_data, start_date, stop_date, input_stop):
    ...
    if input_stop == datetime.now().date():
        ...
        st.metric("Casi", numerize.numerize(int(cases[-1]), 2), util.mysign(delta_cases[-1])+str(delta_cases[-1])+' ('+str(round(ratio[-1],1))+'%)', delta_color="inverse")
        if delta_hospital[-1] != 0:
            st.metric("Ospedalizzati", str(hospital[-1]), util.mysign(delta_hospital[-1])+str(delta_hospital[-1])+' ('+str(round(perc_delta_hospital[-1],1))+'%)', delta_color="inverse")
        else:
            st.metric("Ospedalizzati", str(hospital[-1]), delta=None, delta_color="off")
        if delta_icu[-1] != 0:
            st.metric("Terapie Intensive", str(icu[-1]), util.mysign(delta_icu[-1])+str(delta_icu[-1])+' ('+str(round(perc_delta_icu[-1],1))+'%)', delta_color="inverse")
        else:
            st.metric("Terapie Intensive", str(icu[-1]), delta=None, delta_color="off")
        st.metric("Deceduti", str(deaths[-1]), util.mysign(delta_deaths[-1])+str(delta_deaths[-1]), delta_color="inverse")
